...
func Test_BuildSearchOptions(t *testing.T) {
	...
	//Test case 1
	qopts1, _ := BuildSearchOptions(defaultSize, reqOpts1)
	//Type assertion
	...
	//Test case 2
	_, err := BuildSearchOptions(defaultSize, reqOpts2)
	if err == nil {
		t.Fatalf("Error not catched")
	}
	//Test case 3
	qopts3, _ := BuildSearchOptions(defaultSize, reqOpts3)
	if qopts3["size"].(int64) != 128 || qopts3["from"].(int64) != 0 {
		t.Fatalf("Parser returned wrong size or from")
	}
	//Test case 4
	qopts4, _ := BuildSearchOptions(defaultSize, reqOpts4)
	if qopts4["size"].(int64) != defaultSize || qopts4["from"].(int64) != 80 {
		t.Fatalf("Parser returned wrong size or from")
	}
	//Test case 5, when input is empty
	qopts5, _ := BuildSearchOptions(defaultSize, reqOpts5)
