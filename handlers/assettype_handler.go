...
func (ir *Inventory) AssetTypeOptionsHandler(w http.ResponseWriter, r *http.Request) {
	for k, v := range ASSET_TYPE_ACLS {
		w.Header().Set(k, v)
	}
	w.Header().Set("Content-Type", "text/plain")

	data, err := GetOptionsText(ASSET_TYPE_OPTIONS_TMPLT, NewOptionsMethodVarsFromConfig(ir.cfg))
	if err != nil {
...
func (ir *Inventory) AssetTypeListOptionsHandler(w http.ResponseWriter, r *http.Request) {
	for k, v := range ASSET_TYPE_LIST_ACLS {
		w.Header().Set(k, v)
	}
	w.Header().Set("Content-Type", "text/plain")

	data, err := GetOptionsText(ASSET_TYPE_LIST_OPTIONS_TMPLT, NewOptionsMethodVarsFromConfig(ir.cfg))
	if err != nil {
		ir.writeAndLogResponse(w, r, 500, nil, []byte(err.Error()))
	} else {
		ir.writeAndLogResponse(w, r, 200, nil, data.Bytes())
...
func (ir *Inventory) AssetTypePropertiesHandler(w http.ResponseWriter, r *http.Request) {
	var (
		reqVars   = mux.Vars(r)
...
*/
func (ir *Inventory) AssetTypeGetHandler(w http.ResponseWriter, r *http.Request) {
	var (
...
*/
func (ir *Inventory) AssetTypePostHandler(w http.ResponseWriter, r *http.Request) {
