...
type ServiceManager struct {
	...
	inv *handlers.VindaluApiHandler
...
func (sm *ServiceManager) setupGnatsd(gnatOpts server.Options) (err error) {
	...
	if sm.clusterStatus == nil {
		sm.log.Noticef("Cluster status is nil. No routes for gnatsd\n")
		addrs = []string{}
	} else {
		addrs = sm.clusterStatus.ClusterMemberAddrs()
...
func (sm *ServiceManager) startEventProcessor() error {
	...
	case true:
		...
		<-tck.C
		// Assemble server url's
		var svrAddrs []string
		if sm.clusterStatus != nil {
			svrAddrs = []string{}
			for _, addr := range sm.clusterStatus.ClusterMemberAddrs() {
				svrAddrs = append(svrAddrs, fmt.Sprintf("nats://%s:%d", addr, sm.gnatsOpts.Port))
			}
		}

		// For self use localhost
		if len(svrAddrs) <= 1 {
			svrAddrs = []string{fmt.Sprintf("nats://127.0.0.1:%d", sm.gnatsOpts.Port)}
		}

		nclient, err := events.NewNatsClient(svrAddrs, sm.log)
		if err != nil {
			return fmt.Errorf("nats client failed to connect to %s: %s", svrAddrs, err)
		}
